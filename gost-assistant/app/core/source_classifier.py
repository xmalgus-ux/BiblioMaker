...
def classify_entry(entry: str) -> str:
    ...
    Функция не пытается точно распарсить библиографию. Ее задача - дать
    предварительный TYPE для промпта и для окна ручного уточнения типа.
    """
    text = entry.strip()
    norm = text.lower()
...
def analyze_entry(entry: str) -> tuple[str, list[str]]:
    """Определение типа и возможных недостающих данных.

    Возвращает тип источника и список полей, которые эвристически не найдены.
    Эти данные используются как подсказка, окончательное решение остается за
    пользователем и ИИ-агентом.
    """
    entry_type = classify_entry(entry)
    norm = entry.lower()
    missing = []

    # Набор локальных проверок делает правила читаемее и упрощает добавление
    # новых типов источников без переписывания всей функции.
    def has_author() -> bool:
